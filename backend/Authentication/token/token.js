...
const adminValidate = (req, res, next) => {
  try {
    ...
    jwt.verify(token, 'secret', async (err, decoded) => {
      ...
      // Retrieve user data from the database
      const query = `SELECT * FROM userAuth WHERE user_id = ? and role = 'admin'`;
      await connection.query(query, [decoded._id], (err, result) => {
        if (err) {
          console.error("Error retrieving user data:", err.sqlMessage || err.message);
          return res.sendStatus(500);
...
const data_analystValidate = (req, res, next) => {
  try {
    // Retrieve the token from the cookie
    const token = req.cookies.token_auth;
    ...
    jwt.verify(token, 'secret', async (err, decoded) => {
      ...
      // Retrieve user data from the database
      const query = `SELECT * FROM userAuth WHERE user_id = ? and role = 'data_analyst'`;
      await connection.query(query, [decoded._id], (err, result) => {
        if (err) {
          console.error("Error retrieving user data:", err.sqlMessage || err.message);
          return res.sendStatus(500);
...
export { generateToken, userValidate, adminValidate, data_analystValidate, setTokenCookie };
